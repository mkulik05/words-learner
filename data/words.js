...
// let fill = async (line) => {
//   b++
//   let request_word = line[0]
//   try {
// let res = await fetch("https://translate.google.by/_/TranslateWebserverUi/data/batchexecute?rpcids=MkEWBc&f.sid=-7037657782215732512&bl=boq_translate-webserver_20210908.10_p0&hl=ru&soc-app=1&soc-platform=1&soc-device=1&_reqid=1348120&rt=c", {
//   "headers": {
//     "accept": "*/*",
...
//     console.log("\n", res, "\n")
//     // let partOFspeech = res[0]
//     // res = res[1]
//     // for (let i = 0; i < res.length; i++) {
//     //   translations.push(res[i][0].toLowerCase())
//     // }

//     // data[request_word] = { level: line[2], part: line[1], translations: translations, partOFspeech: partOFspeech }
//     predata[request_word] = res
//     fs.writeFileSync('predata.json', JSON.stringify(predata))
//     console.log(request_word)
//   } catch (e) {
...
let get_data = async (data, i) => {
  if (i < data.length) {
    console.log(i)
    let d = data[i].split(" ")
    // if (fs.existsSync('predata.json')) {
    //   let predata = JSON.parse(fs.readFileSync('predata.json'))
    //   if (Object.keys(predata).includes(d[0])) {
    //     get_data(data, i+1)
    //   } else {
    //     console.log("forgot", d[0])
    //   }
    // }
    let request_word = d[0]
    try {
      // https://translate.google.by/_/TranslateWebserverUi/data/batchexecute?rpcids=MkEWBc&f.sid=-7037657782215732512&bl=boq_translate-webserver_20210908.10_p0&hl=ru&soc-app=1&soc-platform=1&soc-device=1&_reqid=1348120&rt=c
...
let parse = (word, data) => {
  ...
  try {
    ...
    // d = d[0]
    // console.log(JSON.stringfity(d, null, 2), word,typeof d)
